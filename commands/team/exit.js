...
const { exitTeam, exitAllTeam } = require("../../scripts/TeamData");
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("소환사")
        .setDescription("소환사를 맨션해주세요")
    )
    .addBooleanOption((option) =>
      option.setName("모두")
        .setDescription("모든 소환사를 제외해요")
    ),
  async execute(interaction) {

    if (interaction.options.getBoolean("모두")) {
      exitAllTeam();
      
      await interaction.reply(`모든 소환사를 팀 목록에서 제외하였습니다.`);
      return;
    }
